...
def _plot_data(
    id_rep1: list,
    id_rep2: list,
    data_rep1: pd.DataFrame,
    data_rep2: pd.DataFrame,
    prefix="",
):
    """
    Columns:
    ...
    """
    for id in id_rep1:
        data_rep1[data_rep1["Ordre"] == id].sort_values("day")["DFI"].apply(
            lambda x: locale.atof(x) if x != "." else 0
        ).reset_index(drop=True).plot.line()
        plt.savefig(os.path.join(OUTPUT_DATA, f"{prefix}_data_rep1_{id}_dfi.png"))
        plt.clf()
        data_rep1[data_rep1["Ordre"] == id].sort_values("day")["DFI"].apply(
            lambda x: locale.atof(x) if x != "." else 0
        ).cumsum().reset_index(drop=True).plot.line()
        plt.savefig(os.path.join(OUTPUT_DATA, f"{prefix}_data_rep1_{id}_cfi.png"))
        plt.clf()

    for id in id_rep2:
        data_rep2[data_rep2["Ordre"] == id].sort_values("day")["DFI"].apply(
            lambda x: locale.atof(x) if x != "." else 0
        ).reset_index(drop=True).plot.line()
...
def _get_parse_data(
    id_rep1: list, id_rep2: list, data_rep1: pd.DataFrame, data_rep2: pd.DataFrame
):
    rep1 = []
    rep2 = []
    for id in id_rep1:
        rep1.append(
            data_rep1[data_rep1["Ordre"] == id]
            .sort_values("day")["DFI"]
            .apply(lambda x: locale.atof(x) if x != "." else 0)
            .reset_index(drop=True)
        )
    for id in id_rep2:
        rep2.append(
            data_rep2[data_rep2["Ordre"] == id]
            .sort_values("day")["DFI"]
            .apply(lambda x: locale.atof(x) if x != "." else 0)
            .reset_index(drop=True)
        )
...
def _aggregate_corr(data_rep1: list, data_rep2: list, sim: list, prefix="", type=""):
    ...
    for i in range(min(len(data_rep1), len(sim))):
        corr1.append(
            pd.DataFrame({"data": data_rep1[i], "sim": sim[i]}).corr()["sim"]["data"]
        )
    for i in range(min(len(data_rep2), len(sim))):
        corr2.append(
            pd.DataFrame({"data": data_rep2[i], "sim": sim[i]}).corr()["sim"]["data"]
        )
    ...
    print(
        f"Avg {type} {prefix.upper()} Correlation Total: {(sum(corr1) / len(corr1) + sum(corr2) / len(corr2)) / 2}"
    )
...
def _calc_corr():
    ...
    cc_data_rep1 = cc_data[cc_data["rep"] == 1]
    cc_data_rep2 = cc_data[cc_data["rep"] == 2]
    cc_data_rep1, cc_data_rep2 = _get_parse_data(
        cc_id_rep1, cc_id_rep2, cc_data_rep1, cc_data_rep2
    )
    ...
    dc_data_rep1 = dc_data[dc_data["rep"] == 1]
    dc_data_rep2 = dc_data[dc_data["rep"] == 2]
    dc_data_rep1, dc_data_rep2 = _get_parse_data(
        dc_id_rep1, dc_id_rep2, dc_data_rep1, dc_data_rep2
    )
    ...
    cd_data_rep1 = cd_data[cd_data["rep"] == 1]
    cd_data_rep2 = cd_data[cd_data["rep"] == 2]
    cd_data_rep1, cd_data_rep2 = _get_parse_data(
        cd_id_rep1, cd_id_rep2, cd_data_rep1, cd_data_rep2
    )
    ...
    dd_data_rep1 = dd_data[dd_data["rep"] == 1]
    dd_data_rep2 = dd_data[dd_data["rep"] == 2]
    dd_data_rep1, dd_data_rep2 = _get_parse_data(
        dd_id_rep1, dd_id_rep2, dd_data_rep1, dd_data_rep2
    )
